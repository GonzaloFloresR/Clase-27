...
router.get("/:uid",async(req, res)=>{
    ...
    try {
        ...
        if(usuario.rol == "user"){
            ...
            res.setHeader("Content-Type","application/json");
            return res.status(200).json({"status":`Rol del usuario ${usuario.first_name} ${usuario.last_name} actualizado`});
            ...
            res.setHeader("Content-Type","application/json");
            return res.status(200).json({"status":`usuario actualizado ${usuario.first_name} ${usuario.last_name}`});
        } else {
            let rol = usuario.rol;
            res.setHeader("Content-Type","application/json");
